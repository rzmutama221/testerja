...
// ─── Component ───────────────────────────────────────────────────────────────
export default function AdminNetflixPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/netflix');
  ...
  const fetchProcessingOrders = async () => {
    try {
      ...
      const json = await res.json();
      if (json.success) {
        const netflixOrders = (json.data || []).filter((o: ProcessingOrder) =>
          o.variant?.product?.name?.toLowerCase().includes('netflix') ||
          o.variant?.product?.slug?.toLowerCase().includes('netflix')
        );
        setProcessingOrders(netflixOrders);
      }
    } catch { /* ignore */ }
  };
  ...
  const handleDeleteAccount = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/netflix/${id}`, { method: 'DELETE' });
      const json = await res.json();
  ...
  const handleAssignOrder = async () => {
    ...
    try {
      const res = await fetch('/api/admin/netflix/slots', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ slotId: assignSlotId, action: 'assign', orderId: selectedOrderId, profileName: assignProfileName || undefined }),
      });
      const json = await res.json();
      if (json.success) { setSuccess('Order berhasil di-assign'); setAssignSlotId(null); setSelectedOrderId(''); setAssignProfileName(''); fetchData(); }
        ...
        {[1,2].map(i => (
          <div key={i} style={{ background: '#2c2c2c', borderRadius: '12px', padding: '20px', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px' }}>
              {[1,2,3,4,5].map(j => <div key={j} style={{ height: '120px', background: '#3f3f46', borderRadius: '8px' }} />)}
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div style={{ padding: '24px', background: '#171717', minHeight: '100vh', color: 'white' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 700 }}>Panel Slot Netflix</h1>
      ...
      {showAddForm && (
        <form onSubmit={handleAddAccount} style={{ background: '#2c2c2c', borderRadius: '12px', padding: '20px', marginBottom: '24px' }}>
          <h3 style={{ fontSize: '16px', fontWeight: 600, marginBottom: '16px' }}>Tambah Akun Netflix</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
            <div>
              <label style={{ fontSize: '12px', color: '#a1a1aa', display: 'block', marginBottom: '4px' }}>Label *</label>
              <input value={formLabel} onChange={e => setFormLabel(e.target.value)} required placeholder="e.g. Netflix Akun 1" style={{ width: '100%', padding: '10px 12px', background: '#3f3f46', border: '1px solid #52525b', borderRadius: '6px', color: 'white', outline: 'none' }} />
            </div>
            <div>
      ...
      {/* Account List */}
      {accounts.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '48px', color: '#a1a1aa' }}>Belum ada akun Netflix. Klik &quot;Tambah Akun&quot; untuk memulai.</div>
      ) : (
              ...
              {account.slots.map(slot => {
                ...
                if (!slot.isOccupied) {
                  ...
                  return (
                    <div key={slot.id} style={{ border: '2px solid #22c55e', borderRadius: '10px', padding: '16px', background: '#1a2e1a', minHeight: '120px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                      <div>
                        <div style={{ fontSize: '12px', color: '#a1a1aa', marginBottom: '4px' }}>Profil #{slot.profileNumber}</div>
                ...
                return (
                  <div key={slot.id} style={{ border: `2px solid ${isExpired ? '#ef4444' : '#f97316'}`, borderRadius: '10px', padding: '16px', background: isExpired ? '#1a0a0a' : '#2a1a0a', minHeight: '120px' }}>
                    <div style={{ fontSize: '12px', color: '#a1a1aa', marginBottom: '8px' }}>Profil #{slot.profileNumber}{slot.profileName ? ` — ${slot.profileName}` : ''}</div>
                    ...
                    {(slot.deviceBrand || slot.deviceModel) && (
                      <div style={{ fontSize: '11px', color: '#a1a1aa', marginBottom: '4px' }}>
                        Device: {[slot.deviceBrand, slot.deviceModel, slot.deviceType, slot.deviceOs].filter(Boolean).join(' · ')}
                      </div>
                    )}
                    {slot.loginCity && <div style={{ fontSize: '11px', color: '#a1a1aa', marginBottom: '4px' }}>Kota: {slot.loginCity}</div>}
                    <div style={{ fontSize: '11px', color: '#a1a1aa', marginBottom: '4px' }}>Mulai: {slot.startedAt ? new Date(slot.startedAt).toLocaleDateString('id-ID') : '-'}</div>
                    ...
                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                      <button onClick={() => { setEditSlotId(slot.id); setEditData({ profileName: slot.profileName || '', deviceBrand: slot.deviceBrand || '', deviceModel: slot.deviceModel || '', deviceType: slot.deviceType || '', deviceOs: slot.deviceOs || '', loginCity: slot.loginCity || '', expiredAt: slot.expiredAt ? slot.expiredAt.split('T')[0] : '' }); }} style={{ background: '#3f3f46', color: '#e4e4e7', padding: '4px 10px', borderRadius: '4px', border: 'none', cursor: 'pointer', fontSize: '11px' }}>Edit</button>
                    ...
                    {editSlotId === slot.id && (
                      <div style={{ marginTop: '12px', padding: '12px', background: '#3f3f46', borderRadius: '8px' }}>
                        ...
                        {['profileName', 'deviceBrand', 'deviceModel', 'deviceType', 'deviceOs', 'loginCity'].map(field => (
                          <input key={field} placeholder={field} value={editData[field] || ''} onChange={e => setEditData({ ...editData, [field]: e.target.value })} style={{ width: '100%', padding: '6px 8px', background: '#2c2c2c', border: '1px solid #52525b', borderRadius: '4px', color: 'white', marginBottom: '6px', fontSize: '11px', outline: 'none' }} />
                        ))}
            ...
            {assignSlotId && account.slots.some(s => s.id === assignSlotId) && (
              <div style={{ marginTop: '16px', padding: '16px', background: '#3f3f46', borderRadius: '10px' }}>
                ...
                {processingOrders.length === 0 ? (
                  <p style={{ fontSize: '12px', color: '#a1a1aa' }}>Tidak ada order Netflix PROCESSING saat ini.</p>
                ) : (
                  <>
                    ...
                    <input placeholder="Nama Profil (opsional)" value={assignProfileName} onChange={e => setAssignProfileName(e.target.value)} style={{ width: '100%', padding: '8px 12px', background: '#2c2c2c', border: '1px solid #52525b', borderRadius: '6px', color: 'white', marginBottom: '12px', outline: 'none', fontSize: '13px' }} />
                    <div style={{ display: 'flex', gap: '8px' }}>
                      ...
                      <button onClick={() => { setAssignSlotId(null); setSelectedOrderId(''); setAssignProfileName(''); }} style={{ background: '#2c2c2c', color: '#a1a1aa', padding: '8px 16px', borderRadius: '6px', border: '1px solid #52525b', cursor: 'pointer', fontSize: '13px' }}>Batal</button>
                    </div>
                  </>
                )}
              </div>
